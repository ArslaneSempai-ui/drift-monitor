 ...
 */
export function bornesDeBandes(reference, bandes) {
...
export function bande(v, bornes) {
 ...
 */
export function psi(partsRef, partsFenetre) {
    let s = 0;
 ...
 */
export function annee(r, avecDerive, depart = 20260818) {
 ...
 */
export function comportement(r, tirages = 300) {
    const u = graine(4242);
    const reference = Array.from({ length: 20_000 }, () => normale(u));
    const bornes = bornesDeBandes(reference, r.bandes);
    const partsRef = parts(reference, bornes);
    const tirerFenetre = (decale) => psi(partsRef, parts(Array.from({ length: r.fenetre }, () => normale(u) + decale), bornes));
    ...
    for (let k = 0; k < tirages; k++) {
        ...
        let vu = -1;
        for (let t = 0; t < r.controlesParAn; t++) {
            if (tirerFenetre(r.deplacement) >= r.seuil) {
                vu = t;
                break;
            }
        }
...
export function seuilPourBudget(r, faussesParAn, tirages = 200) {
    let bas = 0.01, haut = 1;
    for (let i = 0; i < 14; i++) {
        const mid = (bas + haut) / 2;
        const c = comportement({ ...r, seuil: mid }, tirages);
        if (c.faussesAlertesParAn > faussesParAn)
            bas = mid;
        else
            haut = mid;
    }
 ...
 */
export function rubans(r, tirages = 120) {
    const cle = `${r.fenetre}|${r.bandes}|${r.deplacement}|${tirages}`;
    if (cacheRubans?.cle === cle)
        return cacheRubans.valeur;
    const valeur = calculerRubans(r, tirages);
    cacheRubans = { cle, valeur };
    return valeur;
}
function calculerRubans(r, tirages) {
    ...
    const calme = { bas: [], haut: [] };
    const derive = { bas: [], haut: [] };
    for (const n of fenetres) {
        const a = centiles(n, 0), b = centiles(n, r.deplacement);
        calme.bas.push(a.bas);
        calme.haut.push(a.haut);
        derive.bas.push(b.bas);
        derive.haut.push(b.haut);
    }
 ...
 */
export function fenetreSeparante(r) {
